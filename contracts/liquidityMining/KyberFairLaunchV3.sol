...
import {IKyberFairLaunchV3} from '../interfaces/liquidityMining/IKyberFairLaunchV3.sol';
...
contract KyberFairLaunchV3 is IKyberFairLaunchV3, PermissionAdmin, ReentrancyGuard {
  ...
  function adminWithdraw(uint256 rewardTokenIndex, uint256 amount) external onlyAdmin {
    ...
    _transferReward(rewardToken, msg.sender, amount);
  }
  ...
  function harvestMultiplePools(uint256[] calldata _pids) external override {
    ...
    for (uint256 i = 0; i < totalRewards.length; i++) {
      if (totalRewards[i] > 0) {
        _transferReward(IERC20Ext(rTokens[i]), account, totalRewards[i].div(multipliers[i]));
      }
    }
  }
  ...
  function _updateUserReward(
    ...
    for (uint256 i = 0; i < rTokensLength; i++) {
      ...
      if (shouldHarvest && _pending > 0) {
        uint256 _amount = _pending.div(multipliers[i]);
        _transferReward(IERC20Ext(rewardTokens[i]), _to, _amount);
        emit Harvest(_to, _pid, rewardTokens[i], _amount, _getBlockTime());
      }
    }
  }
