...
contract('ProposalValidator', function (accounts) {
  ...
  it("should return false for proposal cancellations regardless of values", async() => {
    ...
    assert.isFalse(
      await proposalValidator.validateProposalCancellation(governance.address, new BN(1), daoOperator)
    );
  ...
  describe("binary proposal logic", async() => {
    ...
    it("should return false for isQuorumValid if proposal is not binary", async() => {
      ...
      assert.isFalse(
        await proposalValidator.isQuorumValid(
          governance.address,
          proposalId
        )
      );
    ...
    it("should return false for isVoteDifferentialValid if proposal is not binary", async() => {
      ...
      await governance.setProposalType(proposalId, false);
      assert.isFalse(
        await proposalValidator.isVoteDifferentialValid(
          governance.address,
          proposalId
        )
      );
    ...
    it("should return false for isBinaryPassed if proposal is not binary", async() => {
      ...
      await governance.setProposalType(proposalId, false);
      assert.isFalse(
        await proposalValidator.isBinaryProposalPassed(
          governance.address,
          proposalId
        )
      );
